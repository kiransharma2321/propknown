...
function scoreColor(s: number) { return s >= 9 ? "#22c55e" : s >= 8 ? "#7A5C1A" : "#94a3b8"; }
...
function SubmissionCard({ s }: { s: Submission }) {
  ...
  const waMsg = `https://wa.me/917013016003?text=${encodeURIComponent(`Hi, I'm interested in "${s.title}" at ${s.area}, ${s.city}. Asking: ${s.priceDisplay}`)}`;
  ...
  return (
    <div className="card-dark rounded-2xl overflow-hidden group flex flex-col">
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <p className="font-playfair text-xl font-bold mb-2" style={{ color:"var(--gold-text)" }}>{s.priceDisplay}</p>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 text-[10px] font-semibold py-1.5 rounded-lg mb-2 border transition-colors duration-200"
          style={added
            ? { background:"rgba(214,166,62,0.15)", color:"var(--gold-text)", borderColor:"rgba(214,166,62,0.5)" }
...
function BuyPageInner({ initialSubmissions }: { initialSubmissions: Submission[] }) {
  ...
  const sel = "input-dark text-sm py-2.5 px-3 pr-8 appearance-none";
  const inp = "input-dark text-sm py-2.5 px-3";

  return (
    <div className="pt-32 pb-20 bg-white min-h-screen">
      <div className="max-w-7xl mx-auto px-6">
        <div className="mb-10">
          <p className="text-sm tracking-widest uppercase mb-2 font-semibold" style={{ color:"var(--gold-text)" }}>Property Search</p>
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-8 shadow-sm">
          <div className="flex flex-wrap gap-3 items-end">
            ...
            {isSearchActive && (
              <button onClick={clearAll} className="text-sm text-gray-500 hover:text-gray-800 flex items-center gap-1 border border-gray-300 rounded-lg px-3 py-2.5 transition-colors duration-200 whitespace-nowrap">
                <X size={13} /> Clear
              </button>
            )}
          </div>
        ...
        {filteredSubs.length > 0 && (
          <div className="mb-10">
            <div className="flex items-center gap-3 mb-5">
              <h2 className="heading-h3">Owner-Listed Properties</h2>
                ...
                return (
                  <div key={p.id} className="card-dark rounded-2xl overflow-hidden group flex flex-col">
                    ...
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <p className="font-playfair mb-2">
                      ...
                      <div className="mt-auto flex gap-2">
                        ...
                        <Link href={`/buy/${p.id}`} className="btn-secondary flex-1 justify-center text-xs py-2">
                          <Building2 size={12} /> Details
                        </Link>
                      </div>
                    </div>
        ...
        {filtered.length === 0 && filteredSubs.length === 0 && isSearchActive && (
          <div className="text-center py-24">
            <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6" style={{ background:"rgba(214,166,62,0.1)", border:"1px solid rgba(214,166,62,0.3)" }}>
              <Search size={32} style={{ color:"var(--gold-text)" }} />
            </div>
            ...
            <p className="text-gray-500 text-base mb-8 max-w-md mx-auto">
              WhatsApp Raghu on <strong>70130 16003</strong> and we will find your perfect property for you.
            </p>
            <a href="https://wa.me/917013016003?text=Hi%20Raghu%2C%20I%20am%20looking%20for%20a%20property%20and%20need%20your%20help."
              target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 text-white font-semibold px-8 py-3 rounded-lg shadow-md transition-colors duration-200 hover:opacity-90 text-sm"
              ...
              WhatsApp Raghu — 70130 16003
            </a>
            <div className="mt-6">
              <button onClick={clearAll} className="btn-tertiary">← Clear filters and show all listings</button>
            </div>
          </div>
        )}
      </div>
