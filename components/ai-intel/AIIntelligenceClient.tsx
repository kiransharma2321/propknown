...
import { HOT_MARKETS, PROPERTY_TYPES, slugify } from "@/lib/hotMarkets";
...
function AIIntelligenceInner() {
  ...
  // "Get Full Location Overview" -- a separate action from the single-type "Analyze" above,
  // hitting /api/market-intel/overview instead. Shares the same gate UI (limitReached/usage), so
  // lastAction tracks which one to re-run once AiIntelRegisterGate unlocks the visitor.
  const [overviewResults, setOverviewResults] = useState<Record<string, Record<string, unknown>> | null>(null);
  const [overviewLoading, setOverviewLoading] = useState(false);
  const [overviewCached,  setOverviewCached]  = useState(false);
  const [lastAction, setLastAction] = useState<"single" | "overview">("single");
  ...
  const analyze = async () => {
    ...
    setLoading(true); setError(""); setResult(null); setOverviewResults(null); setShowEMI(false); setLoadStep(0); setLimitReached(false);
  ...
  // ── Get Full Location Overview ───────────────────────────────────────────────
  // Same location fields, no property-type selection needed -- returns every type from
  // PROPERTY_TYPES at once via /api/market-intel/overview, which either serves an instant free
  // cache hit (hot markets) or fires the per-type calls in parallel and consumes exactly one
  // free check for the whole location. Does not touch the single-type `analyze()` flow above.
  const analyzeOverview = async () => {
    if (!cityValue) { setError("Please enter a city or country to analyze."); return; }
    setLastAction("overview");
    setOverviewLoading(true); setError(""); setOverviewResults(null); setResult(null); setLoadStep(0); setLimitReached(false);
    ...
    try {
      const res  = await fetch("/api/market-intel/overview", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ city: cityValue, area: areaValue }),
      });
      const data = await res.json();
      if (res.status === 403 && data.error === "usage_limit") {
        setLimitReached(true);
        ...
        setOverviewResults(data.results);
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              ...
              {limitReached ? (
                <div className="mt-4">
                  <AiIntelRegisterGate
                    searchContext={`${areaValue ? `${areaValue}, ${cityValue}` : cityValue} (${lastAction === "overview" ? "Full Location Overview" : PROPERTY_TYPES.find(t => t.value === propType)?.label ?? propType})`}
                    onUnlocked={() => { setLimitReached(false); if (lastAction === "overview") analyzeOverview(); else analyze(); }}
                  />
                </div>
              ) : (
                <>
                  {error && (
                    <div className="flex items-start gap-2 text-red-600 text-sm mt-3 bg-red-50 border border-red-200 rounded-lg px-3 py-2.5">
                      ...
                      {error.includes("WhatsApp") ? (
                        ...
                        <button
                          onClick={lastAction === "overview" ? analyzeOverview : analyze}
                  ...
                  <button onClick={analyze} disabled={loading || overviewLoading}
                  ...
                  <button onClick={analyzeOverview} disabled={loading || overviewLoading}
                    className="btn-secondary w-full justify-center mt-2.5 py-3 text-sm disabled:opacity-60">
                    {overviewLoading
                      ? <><Loader2 size={16} className="animate-spin" />{LOADING_STEPS[loadStep]}</>
                      : <>See All Property Types for This Location</>}
                  </button>
            ...
            {/* ── Full Location Overview Results ──────────────────────────────── */}
            {overviewResults && (
              <div className="mt-6">
                <div className="flex items-center justify-between gap-2 mb-4">
                  <h2 className="heading-h3 text-lg">All Property Types</h2>
                  {overviewCached && (
                    <span className="text-[10px] font-bold text-green-700 bg-green-50 border border-green-200 rounded-full px-2.5 py-0.5">
                      Instant · Precomputed
                    </span>
                  )}
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {PROPERTY_TYPES.map((pt) => (
                    <PropertyTypeSummaryCard
                      ...
                      data={overviewResults[pt.value] as PropertyTypeSummaryData ?? { available: false }}
                    />
                  ))}
                </div>
              </div>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
              ...
              <div className="flex flex-wrap gap-x-3 gap-y-1 mt-3 pt-3 border-t border-gray-100">
                <span className="text-gray-400 text-[11px]">Instant overview:</span>
                {HOT_MARKETS.map((m) => (
                  <Link
                    key={`overview-${m.city}-${m.area}`}
                    href={`/ai-intelligence/${slugify(m.city)}/${slugify(m.area)}`}
                    className="text-[11px] text-gray-500 hover:text-[#7A5C1A] hover:underline"
                  >
                    {m.area}
                  </Link>
                ))}
