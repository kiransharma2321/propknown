...
const ROLE_LABELS: Record<string, { label: string; color: string; style?: CSSProperties }> = {
  master:  { label: ROLE_NAME.master,  color: "text-navy", style: { background: "var(--gold)" } },
  manager: { label: ROLE_NAME.manager, color: "bg-blue-500 text-white" },
  agent:   { label: ROLE_NAME.agent,   color: "bg-zinc-600 text-white" },
  super_admin:       { label: ROLE_NAME.super_admin,       color: "text-navy", style: { background: "var(--gold)" } },
  chairman:          { label: ROLE_NAME.chairman,          color: "text-navy", style: { background: "var(--gold)" } },
  managing_director: { label: ROLE_NAME.managing_director, color: "text-navy", style: { background: "var(--gold)" } },
  ceo:               { label: ROLE_NAME.ceo,               color: "text-navy", style: { background: "var(--gold)" } },
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center gap-3 mb-8">
          <Link href="/admin/dashboard" className="text-gray-500 hover:text-gray-900"><ArrowLeft size={18} /></Link>
          <div>
            <h1 className="font-playfair text-gray-900 text-xl font-bold flex items-center gap-2"><Shield size={18} style={{ color: "var(--gold)" }} /> User Management</h1>
            <p className="text-gray-500 text-sm">Create and manage admin team members (Master Admin only)</p>
          </div>
        </div>

        {error && <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-5 text-red-600 text-sm">{error}</div>}
        {success && <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-5 text-green-600 text-sm">{success}</div>}

        {/* Add user form */}
        <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-6">
          <h2 className="font-playfair text-gray-900 font-semibold text-sm mb-4">Add Team Member</h2>
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <label className="text-gray-500 text-xs mb-1 block">Full Name</label>
              <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                placeholder="John Doe" className="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-[#D6A63E]" />
            </div>
            <div>
              <label className="text-gray-500 text-xs mb-1 block">Email</label>
              <input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
                placeholder="agent@propknown.com" className="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-[#D6A63E]" />
            </div>
            <div>
              ...
              <div className="relative">
                <input type={showPw ? "text" : "password"} value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))}
                  placeholder="Strong password" className="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-lg px-3 py-2 pr-9 focus:outline-none focus:border-[#D6A63E]" />
                <button type="button" onClick={() => setShowPw(p => !p)} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700">
            ...
            <div>
              ...
              <select value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))}
                className="w-full bg-white border border-gray-200 text-gray-700 text-sm rounded-lg px-3 py-2 focus:outline-none">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-200">
            <p className="font-playfair text-gray-900 text-sm font-semibold">Team Members ({users.length})</p>
          </div>
          {loading ? (
            <div className="p-8 text-center text-gray-500 text-sm">Loading...</div>
          ) : users.length === 0 ? (
            <div className="p-8 text-center text-gray-400 text-sm">No team members yet. Create your first below.</div>
          ) : (
              ...
              {users.map(u => {
                ...
                return (
                  <div key={u.id} className="flex items-center gap-4 px-5 py-3.5">
                    ...
                    {!u.isActive && <span className="text-xs text-gray-400 bg-gray-100 border border-gray-200 px-2 py-1 rounded-full">Inactive</span>}
                    <p className="text-gray-400 text-xs shrink-0">{new Date(u.createdAt).toLocaleDateString("en-IN")}</p>
                    {u.role !== "master" && u.isActive && (
                      <button onClick={() => deactivate(u.id, u.name)} className="text-gray-400 hover:text-red-600 transition-colors ml-2">
                        <Trash2 size={14} />
                      </button>
                    )}
                  </div>
