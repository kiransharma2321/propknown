...
export async function POST(req: NextRequest) {
  ...
  if (!market.available) {
    return respond({
      ...
      unit:        resolvedUnit,
      message:     `We don't have reliable live market data for ${location.trim()} right now, so we can't confidently assess this price. Please try again in a moment, or WhatsApp our team on 70130 16003 for a manual opinion.`,
    });
  }
