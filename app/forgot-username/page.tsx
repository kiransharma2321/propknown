...
import { UserSearch, ArrowLeft, CheckCircle } from "lucide-react";
...
export default function ForgotUsernamePage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage(data.message || "If this email is registered, you'll receive an email with your username shortly.");
      setSent(true);
    } catch {
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-full bg-gold-500/20 border border-gold-500/40 flex items-center justify-center">
              <UserSearch size={18} className="text-gold-400" />
            </div>
            <div>
              <h1 className="font-playfair text-white font-semibold">Forgot Username</h1>
            ...
            <form onSubmit={submit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  required
                />
                <p className="text-[10px] text-zinc-500 mt-1.5">Your username is the same email address you registered with.</p>
              </div>
