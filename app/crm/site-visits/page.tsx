...
export default function SiteVisitsPage() {
  ...
  const STATUS_COLORS: Record<string, string> = { scheduled: "bg-blue-100 text-blue-700", completed: "bg-green-100 text-green-700", cancelled: "bg-red-100 text-red-700", no_show: "bg-amber-100 text-amber-700" };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-3xl mx-auto p-6">
        ...
        <h1 className="font-playfair text-gray-900 text-xl font-bold mb-1">Site Visits</h1>
        <p className="text-gray-500 text-sm mb-6">Schedule, assign, and track visit outcomes.</p>
        ...
        {showForm && (
          <div className="bg-white border border-gray-200 rounded-2xl p-5 mb-6 space-y-3">
            ...
            <input type="datetime-local" value={form.scheduledAt} onChange={e => setForm(f => ({ ...f, scheduledAt: e.target.value }))} className="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-lg px-3 py-2" />
            <input value={form.assignedTo} onChange={e => setForm(f => ({ ...f, assignedTo: e.target.value }))} placeholder="Assign to (executive name)" className="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-lg px-3 py-2" />
            <input value={form.mapsLink} onChange={e => setForm(f => ({ ...f, mapsLink: e.target.value }))} placeholder="Google Maps link (optional)" className="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-lg px-3 py-2" />
        ...
        {loading ? <p className="text-gray-500 text-sm">Loading…</p> : visits.length === 0 ? (
          <p className="text-gray-400 text-sm text-center py-10">No site visits scheduled yet.</p>
        ) : (
          <div className="space-y-3">
            {visits.map(v => (
              <div key={v.id} className="bg-white border border-gray-200 rounded-2xl p-4">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-gray-900 text-sm font-medium">{v.lead.name} {v.property && <span className="text-gray-500">· {v.property.title}</span>}</p>
                  <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${STATUS_COLORS[v.status] ?? "bg-gray-100 text-gray-700"}`}>{v.status.replace("_", " ")}</span>
                </div>
