...
export default function ExecutiveDashboardPage() {
  ...
  const Kpi = ({ icon, label, value, sub }: { icon: React.ReactNode; label: string; value: React.ReactNode; sub?: string }) => (
    <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
      <div className="flex items-center gap-2 mb-2 text-zinc-400">{icon}<span className="text-xs">{label}</span></div>
      <p className="text-white text-2xl font-bold">{value}</p>
      {sub && <p className="text-zinc-500 text-xs mt-1">{sub}</p>}
    </div>
  );

  const EmptyState = ({ text }: { text: string }) => <p className="text-zinc-600 text-sm text-center py-6">{text}</p>;
    ...
    <div className="min-h-screen" style={{ background: "var(--navy)" }}>
      <div className="max-w-6xl mx-auto p-6">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link href="/crm/dashboard" className="flex items-center gap-1.5 text-xs text-zinc-400 hover:text-white border border-white/10 rounded-lg px-3 py-1.5">
        ...
        {loading || !stats ? (
          ...
          <>
            ...
            <div className="grid md:grid-cols-2 gap-5 mb-6">
              ...
              <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
                <div className="flex items-center gap-2 mb-3"><Award size={14} style={{ color: "var(--gold)" }} /><h2 className="text-white font-semibold text-sm">Top Sales Executives</h2></div>
                  ...
                  <div className="space-y-2">
                    {stats.topExecutives.map(e => (
                      <div key={e.name} className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-zinc-400">{e.won} won / {e.total} leads</span>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-5 mb-6">
              <div className="flex items-center gap-2 mb-3"><Megaphone size={14} style={{ color: "var(--gold)" }} /><h2 className="text-white font-semibold text-sm">Marketing ROI</h2></div>
                ...
                <div className="space-y-2">
                  {stats.campaignRoi.map(c => (
                    <div key={c.name} className="flex items-center justify-between text-sm">
                      <span className="text-zinc-300">{c.name} ({c.channel})</span>
                      <span className="text-zinc-400">{c.leads} leads · {c.cpl != null ? `₹${c.cpl}/lead` : "no leads attributed yet"}</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            {/* Recent Activity */}
            <div className="bg-white/5 border border-white/10 rounded-2xl p-5 mb-6">
              <div className="flex items-center gap-2 mb-3"><Activity size={14} style={{ color: "var(--gold)" }} /><h2 className="text-white font-semibold text-sm">Recent Activity</h2></div>
              {stats.recentActivity.length === 0 ? <EmptyState text="No activity yet." /> : (
                <div className="space-y-2">
                  {stats.recentActivity.map(l => (
                    <Link key={l.id} href={`/crm/leads/${l.id}`} className="flex items-center justify-between text-sm hover:text-white">
                      <span className="text-zinc-300">{l.name} <span className="text-zinc-600">via {l.source}</span></span>
                      <span className="text-zinc-500 text-xs">{l.status} · {new Date(l.updatedAt).toLocaleDateString("en-IN")}</span>
                    </Link>
                  ))}
                </div>
              )}
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2"><Sparkles size={14} style={{ color: "var(--gold)" }} /><h2 className="text-white font-semibold text-sm">AI Recommendations</h2></div>
              ...
              {insights ? (
                ...
                <p className="text-amber-400 text-sm">{insightsError}</p>
              ) : (
