    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-gray-50">
      <div className="w-full max-w-md">
        <div className="flex flex-col items-center mb-10">
          <PKLogo />
          <span className="text-[9px] tracking-[0.4em] text-gray-400 mt-3">CRM PORTAL</span>
        </div>
        ...
        <div className="card-dark p-8">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: "rgba(214,166,62,0.12)", border: "1px solid rgba(214,166,62,0.4)" }}>
              <Activity size={18} style={{ color: "var(--gold-text)" }} />
            </div>
            <div>
              <h1 className="heading-h3">CRM Login</h1>
              <p className="text-gray-500 text-xs">Lead management portal</p>
            </div>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button type="submit" disabled={loading} className="btn-primary w-full justify-center py-3 disabled:opacity-60">
              {loading ? <span className="flex items-center gap-2"><span className="w-4 h-4 border-2 border-navy/30 border-t-navy rounded-full animate-spin" />Signing in...</span> : <span className="flex items-center gap-2"><Lock size={16} />Sign In to CRM</span>}
            </button>
